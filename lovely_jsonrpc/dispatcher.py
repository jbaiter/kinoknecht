...
from lovely_jsonrpc import DEFAULT_JSON_IMPL, JSONImplementationNotFound
import functools
...
def get_getter(instance, name):
    def generic_getter():
        return getattr(instance, name)
    return generic_getter

def get_setter(instance, name):
    def generic_setter(value):
...
class JSONRPCDispatcher(object):
    ...
    def register_instance(self, instance):
        """Registers all public attributes of class instance to dispatcher"""
        for attribute in dir(instance):
            if attribute.startswith('_') is False:
                attr_obj = getattr(instance, attribute)
                if callable(attr_obj):
                    # All public methods
                    self.register_method(attr_obj, name=attribute)
                else:
                    # Create getter/setter for public properties
                    # TODO: Ensure we are only dealing with simple datatypes
                    #       that are JSON compatible in getting and setting
                    #       (strings, numbers, lists, dicts). Be more
                    #       restrictive about setters.
                    self.register_method(get_getter(instance, attribute),
                        name="get_%s" % attribute)
                    self.register_method(get_setter(instance, attribute),
                        name="set_%s" % attribute)
    ...
    def register_method(self, function, name=None):
        ...
        if not name in self.methods:
            ...
            _log.warning('Attribute name conflict -- "%s" must be removed before\
                    attribute of the same name can be added' % name)
